...
import { addPathBorders, addBorderStyles }
...
export function highlightPath(path) {
    ...
    // Add path borders using the separate module
    addPathBorders(path, (index) => document.querySelector(`[data-index="${index}"]`));
}
...
function formatForMathJax(value) {
    ...
    if (window.MathJax && cell.innerHTML.includes(' (typeof value === 'string') {
...
export function renderGrid(gridEntries, options = {}) {
    ...
    // Make sure any existing borders are cleared 
...
export function renderGrid(gridEntries, options = {}) {
    ...
    // Make sure border styles are added
    addBorderStyles();
